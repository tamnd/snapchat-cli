...
describes what users get. Pick a format with `-o`, or let snap choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
snap <command> -o table   # aligned columns for reading
snap <command> -o jsonl   # one JSON object per line, for piping
snap <command> -o json    # a single JSON array
snap <command> -o csv     # spreadsheet friendly
snap <command> -o tsv     # tab-separated
snap <command> -o url     # just the URL column
snap <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
snap <command> --fields id,title,url
```
...
```bash
snap <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
snap <command>            # a table, because this is a terminal
snap <command> | wc -l    # JSONL, because this is a pipe
```
