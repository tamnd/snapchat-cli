// Package cli assembles the snap command tree from the snapchat
...
// NewApp assembles the kit application from the snapchat domain. The
...
//
// To add a command, declare it in snapchat/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := snapchat.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(snapchat.Domain{}).Register(app)
